...
export default function TagScreen() {
  ...
  const fetchEmployee = async (activeTab: string) => {
    ...
    if (activeTab == "Active") {
      const data = await GetTagListApi(param, setLoading)
      setEmployeeData(data?.data?.data)
      console.log(data?.data?.data, 'this is instide employee')
    } else {
      const data = await GetDeletedTagApi(param, setLoading)
        ...
        <View style={styles.cardTopRow}>
          ...
          {activeTab == "Active" &&
            <View style={styles.cardBottomRow}>
              <TouchableOpacity style={styles.iconBtn} onPress={() => nav.navigate(ScreenNameEnum.TagDetail, { item: item })}>
    ...
    <SafeAreaView edges={['top']} style={styles.container}>
      ...
      {activeTab == "Active" &&
        <TouchableOpacity style={styles.fab}
          ...
          onPress={() => {
            nav.navigate(ScreenNameEnum.AddTag)
          }}
        >
          <Image
