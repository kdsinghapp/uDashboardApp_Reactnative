...
export default function EmployeeScreen() {
  ...
  const fetchEmployee = async (activeTab: string) => {
    const param = {
      token: isLogin?.token,
      url: activeTab == "Active" ? endpointCustomer?.GetTeamsList : endpointCustomer?.GetDeletedTeamsList
    }
  ...
  const handleDelete = async () => {
    ...
    const param = {
      token: isLogin?.token,
      url: endpointCustomer?.DeleteTeams + selectedItem?.id
    }
  ...
  const handleRestore = async () => {
    ...
    const param = {
      token: isLogin?.token,
      url: endpointCustomer?.RestoreTeams + selectedItem?.id,
    }
  ...
  const renderCard = ({ item }: any) => {
    return (
      <CommonCard
        ...
        onPress={() => nav.navigate(ScreenNameEnum.TeamsDetail, { item })}
        actions={
          activeTab === "Active"
            ? [
              {
              ...
              },
              {
                ...
                onPress: () => nav.navigate(ScreenNameEnum.AddTeams, { item }),
              },
              {
  ...
  return (
    <SafeAreaView edges={['top']} style={styles.container}>
      ...
      <RestoreModal
        ...
        title="Delete Employee?"
        message="Do you want to restore this Team?"
        cancelText="No"
