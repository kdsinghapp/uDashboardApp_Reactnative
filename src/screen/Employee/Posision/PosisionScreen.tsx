...
export default function PosisionScreen() {
  const [activeTab, setActiveTab] = useState<"Active" | "Deleted">("Active");
  const [deleteModalVisible, setDeleteModalVisible] = useState(false);
  const [loading, setLoading] = useState(false)
  const [PosisionData, setPosisionData] = useState([])
  ...
  useFocusEffect(
    useCallback(() => {
      setActiveTab("Active");
      fetchPosision("Active");
    }, [])
  );
  useEffect(() => {
    fetchPosision(activeTab);
  }, [activeTab]);

  const filteredData = PosisionData.filter((item: any) => {
  ...
  const fetchPosision = async (activeTab: string) => {
    const param = {
      token: isLogin?.token,
      url: activeTab == "Active" ? endpointCustomer?.GetPosisionList : endpointCustomer?.GetDeletedPosisionList
    }
    const data = await GetApi(param, setLoading)
    setPosisionData(data?.data?.data ?? [])
  ...
  const handleDelete = async () => {
   if (!selectedItem)  return
    const param = {
      token: isLogin?.token,
      url: endpointCustomer?.DeletePosision + selectedItem?.id
    }
  ...
  if (!selectedItem)  return
    const param = {
      token: isLogin?.token,
      url: endpointCustomer?.RestorePosision + selectedItem?.id,
    }
  ...
  const renderCard = ({ item }: any) => {
    return (
      <CommonCard
        ...
        onPress={() => nav.navigate(ScreenNameEnum.PosisionDetail, { item })}
        actions={
          activeTab === "Active"
            ? [
              {
              ...
              },
              {
                ...
                onPress: () => nav.navigate(ScreenNameEnum.AddPosisionScreen, { item }),
              },
              {
    ...
    <SafeAreaView edges={['top']} style={styles.container}>
      ...
      {activeTab == "Active" &&
        <TouchableOpacity style={styles.fab}
          ...
          onPress={() => {
            nav.navigate(ScreenNameEnum.AddPosisionScreen)
          }}
        >
          <Image
      ...
      <DeleteModal
        ...
        message="Are you sure you want to delete this Posision from your list?"
        cancelText="No"
      ...
      <RestoreModal
        ...
        title="Delete Posision?"
        message="Do you want to restore this Posision?"
        cancelText="No"
